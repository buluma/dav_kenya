...
{		
	...
	static function getList($params, $module)
	{
		...
		foreach ($items as &$item) {
			...
			$link_string = ContentHelperRoute::getArticleRoute($item->slug, $item->cat_slug, $item->language);
			if ($params->get('force_itemid', '') != '') {
				if (strpos($link_string, 'Itemid') === false) {
					$link_string .= '&Itemid='.$params->get('force_itemid', '');
				} else {
					$link_string = preg_replace('#Itemid=([0-9]*)#', 'Itemid='.$params->get('force_itemid', ''), $link_string);
				}
			}
			...
			if ($item->category_authorized && (!$show_unauthorized_items || in_array($item->access, $authorised))) {
				...
				$item->link = JRoute::_($link_string);
