...
class JFormFieldStyleSelect extends JFormFieldDynamicSingleSelect
{
	...
	protected function getOptions() 
	{
		...
		foreach ($optionsArray as $option) {
			...
			$translated_option = JText::_('MOD_LATESTNEWSENHANCEDEXTENDED_STYLE_OVERALL_'.$upper_option.'_LABEL');
			
			$description = '';
			if (empty($translated_option) || substr_count($translated_option, 'LATESTNEWSENHANCEDEXTENDED') > 0) {				
				$translated_option = ucfirst($option);
			} else {
				$description = JText::_('MOD_LATESTNEWSENHANCEDEXTENDED_STYLE_OVERALL_'.$upper_option.'_DESC');
